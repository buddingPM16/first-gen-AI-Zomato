...
# Define databse path for UI queries
DB_PATH = os.path.join(os.path.dirname(__file__), '..', 'phase3_indexing_storage', 'zomato.db')

# Add previous phase to path to reuse the LLM generator directly without HTTP overhead
# Alternatively, we could use requests to hit the FastAPI backend we made in Phase 5, 
# but calling the function directly is perfectly fine for this architecture.
...
logo_path = os.path.join(os.path.dirname(__file__), 'zomato_logo.svg')
